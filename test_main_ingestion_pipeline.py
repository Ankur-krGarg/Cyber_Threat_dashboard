...
from test_ner_extractor import perform_hybrid_ner
from test_relation_extractor import extract_relationships_llm
from test_mitre_stix_integrator import enrich_entities_with_mitre_stix
from test_neo4j_persistor import Neo4jPersistor
from test_ingestion import ingest_data
...
def process_document(doc: dict) -> Tuple[List[ThreatEntity], List[ThreatRelationship]]:
    """
    Processes a single document through the full test_KG_pipeline:
...
def ingest_to_neo4j(entities: Sequence[ThreatEntity], relationships: Sequence[ThreatRelationship], db: Neo4jPersistor, dry_run: bool = False):
    """
...
def run_pipeline(chunked_json_path: str, neo4j_uri: str, neo4j_user: str, neo4j_password: str, dry_run: bool = False, sample_size: int = 0):
    """
    ...
    if sample_size > 0:
        ...
        logger.info(f"Sampling {len(documents)} documents for processing")
    else:
        logger.info("Processing all documents (no sampling).")
    ...
    db = Neo4jPersistor(uri=neo4j_uri, user=neo4j_user, password=neo4j_password, run_id=run_id)

    try:
        # Estimate total time by timing a sample first (optional)
        sample_estimate_size = min(3, total_docs)
        if sample_estimate_size > 0:
            logger.info(f"Estimating total time by processing sample of {sample_estimate_size} documents...")
            start_sample = time.time()
            for i in range(sample_estimate_size):
                process_document(documents[i])
            end_sample = time.time()
            avg_per_doc = (end_sample - start_sample) / sample_estimate_size
...
if __name__ == "__main__":
    ...
    parser.add_argument("--sample-size", type=int, default=0, help="Process only a sample of documents, 0 for all")
    args = parser.parse_args()
    sample_size = args.sample_size  

    GROQ_API_KEY = os.getenv("GROQ_API_KEY")
    CHUNKED_JSON_PATH = os.getenv("CHUNKED_JSON_PATH", r"D:\Tiny_threat_dashboard\processed\threats_chunked_clean.json")
    ...
    run_pipeline(CHUNKED_JSON_PATH, NEO4J_URI, NEO4J_USER, NEO4J_PASSWORD, dry_run=args.dry_run, sample_size=args.sample_size)
    end_time = time.time()
